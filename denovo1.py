...
from modules import get_ion_index
...
class IonCNNDenovo(object):
    ...
    def _beam_search(self, model_wrapper: InferenceModelWrapper,
        ...
        batch_precursormass = np.array([x.precursormass for x in feature_dp_batch])
        ...
        batch_precursormass = torch.from_numpy(batch_precursormass).to(device)
        ...
        for feature_index in range(num_features):
            ...
            spectrum_state = (batch_peak_location[feature_index], batch_peak_intensity[feature_index], batch_precursormass[feature_index])
            #
            if config.use_lstm:
        ...
        while True:
            ...
            for entry_index, search_entry in enumerate(active_search_list):
                ...
                for path in current_path_list:
                    ...
                    block_precursormass.append(original_spectrum_tuple[2])
                    if config.use_lstm:
