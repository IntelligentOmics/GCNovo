...
class WorkerTest(object):
    ...
    def __init__(self, args):
        ...
        # we currently use config to store both const & settings
        # the settings should be shown in __init__() to keep track carefully
        self.MZ_MAX = args.MZ_MAX

        self.target_file = args.denovo_input_feature_file
        self.predicted_file = args.denovo_output_file
        self.predicted_format =  config.predicted_format
        self.accuracy_file = args.accuracy_file
        self.denovo_only_file = args.denovo_only_file
        self.scan2fea_file = args.scan2fea_file
        self.multifea_file = args.multifea_file
    ...
    def test_accuracy(self, db_peptide_list=None):
        ...
        logger.info("target_count_total = {0:d}".format(target_count_total))
        logger.info("target_len_total = {0:d}".format(target_len_total))
        logger.info("target_count_db = {0:d}".format(target_count_db))
        logger.info("target_len_db = {0:d}".format(target_len_db))
        logger.info("target_count_db_mass: {0:d}".format(target_count_db_mass))
        logger.info("target_len_db_mass: {0:d}".format(target_len_db_mass))

        logger.info("predicted_count_mass: {0:d}".format(predicted_count_mass))
        logger.info("predicted_count_mass_db: {0:d}".format(predicted_count_mass_db))
        logger.info("predicted_len_mass_db: {0:d}".format(predicted_len_mass_db))
        logger.info("predicted_only: {0:d}".format(predicted_only))

        logger.info("recall_AA_total = {0:.4f}".format(recall_AA_total / target_len_total))
        logger.info("recall_AA_db = {0:.4f}".format(recall_AA_total / target_len_db))
        logger.info("recall_AA_db_mass = {0:.4f}".format(recall_AA_total / target_len_db_mass))
        logger.info("recall_peptide_total = {0:.4f}".format(recall_peptide_total / target_count_total))
        logger.info("recall_peptide_db = {0:.4f}".format(recall_peptide_total / target_count_db))
        logger.info("recall_peptide_db_mass = {0:.4f}".format(recall_peptide_total / target_count_db_mass))
        logger.info("precision_AA_mass_db  = {0:.4f}".format(recall_AA_total / predicted_len_mass_db))
        logger.info("precision_peptide_mass_db  = {0:.4f}".format(recall_peptide_total / predicted_count_mass_db))
    ...
    def _get_predicted(self):
        ...
        with open(self.predicted_file, 'r') as handle:
            ...
            for line in handle:
                ...
                predicted["feature_area"] = line_split[col_feature_area]
