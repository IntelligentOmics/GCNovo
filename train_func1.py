...
from model_gcn import DeepNovoModel, device, InitNet
import time
import math
import logging
...
def  build_model(args, training=True):
    ...
    if os.path.exists(os.path.join(args.train_dir, forward_model_save_name)):
        ...
        if args.use_lstm:
            init_net.load_state_dict(torch.load(os.path.join(args.train_dir, init_net_save_name),
                                                map_location=device))
...
def extract_and_move_data(data):
    ...
    precursormass = precursormass.to(device)
...
def validation(forward_deepnovo, backward_deepnovo,  init_net, valid_loader, args) -> float:
    with torch.no_grad():
        valid_loss = 0
        num_valid_samples = 0
        for data in valid_loader:
            ...
            if args.use_lstm:
                ...
                # forward_logit = forward_deepnovo(batch_forward_ion_index, peak_location, peak_intensity, spectrum_representation, batch_forward_id_input)
                # backward_logit = backward_deepnovo(batch_backward_ion_index, peak_location, peak_intensity, spectrum_representation, batch_backward_id_input)
                forward_logit = forward_deepnovo(batch_forward_ion_index, peak_location, peak_intensity, precursormass)
                backward_logit = backward_deepnovo(batch_backward_ion_index, peak_location, peak_intensity, precursormass)
...
def save_model(forward_deepnovo, backward_deepnovo, init_net, args):
    torch.save(forward_deepnovo.state_dict(), os.path.join(args.train_dir,
                                                           forward_model_save_name))
    torch.save(backward_deepnovo.state_dict(), os.path.join(args.train_dir,
                                                            backward_model_save_name))
    if config.use_lstm:
        torch.save(init_net.state_dict(), os.path.join(args.train_dir,
                                                   init_net_save_name))


def train(args):
    train_set = DeepNovoTrainDataset(args.input_feature_file_train,
    ...
    valid_set = DeepNovoTrainDataset(args.input_feature_file_valid,
                                     args.input_spectrum_file_valid,
                                     args=args)
    ...
    forward_deepnovo, backward_deepnovo, init_net = build_model(args=args)
    ...
    for epoch in range(args.num_epoch):
        ...
        for i, data in enumerate(train_data_loader):
            ...
            if args.use_lstm:
                initial_state_tuple = init_net(spectrum_representation)
                forward_logit, _ = forward_deepnovo(batch_forward_ion_index, peak_location, peak_intensity, precursormass,
                                                    batch_forward_id_input, initial_state_tuple)
                backward_logit, _ = backward_deepnovo(batch_backward_ion_index, peak_location, peak_intensity,precursormass,
                                                      batch_backward_id_input, initial_state_tuple)
            else:
                forward_logit = forward_deepnovo(batch_forward_ion_index, peak_location, peak_intensity, precursormass)
                backward_logit = backward_deepnovo(batch_backward_ion_index, peak_location, peak_intensity, precursormass)
            ...
            total_loss.backward()
            # clip gradient
            # torch.nn.utils.clip_grad_norm_(dense_params, config.max_gradient_norm)
            ...
            if (i + 1) % args.steps_per_validation == 0:
                ...
                validation_loss = validation(forward_deepnovo, backward_deepnovo, init_net, valid_data_loader, args=args)
                ...
                if validation_loss < best_valid_loss:
                    ...
                    print('save model+++++++++++++++++++++++++++++++')
                    save_model(forward_deepnovo, backward_deepnovo, init_net, args=args)
